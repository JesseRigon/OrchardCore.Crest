# Refactoring Summary: OrchardCore.Crest.Workflows Designer Architecture

## Overview

The OrchardCore.Crest.Workflows Designer has been refactored from a standalone Blazor WebAssembly application into a Razor Class Library (RCL) with a separate reference WASM application.

## Changes Made

### 1. OrchardCore.Crest.Workflows.Designer (Converted to RCL)
...
### 2. OrchardCore.Crest.Workflows.Web.Blazor (New Project)

**Purpose:** Reference Blazor WebAssembly application and template for consumers

**Created Files:**
- `OrchardCore.Crest.Workflows.Web.Blazor.csproj` - Blazor WASM project file
...
### 3. OrchardCore.Crest.Workflows Module (Simplified)

**Removed:**
- `Options/ElsaBlazorOptions.cs` - No longer needed
- Project reference to `OrchardCore.Crest.Workflows.Designer`
- Options registration in `CoreStartup.cs`

**Updated:**
- `Features/CoreStartup.cs`:
  - Removed `using OrchardCore.Crest.Workflows.Options`
...
### 4. OrchardCore.Crest.Workflows.Web (Updated)

**Changes:**
- Added project reference to `OrchardCore.Crest.Workflows.Web.Blazor`
...
src/
  modules/
    OrchardCore.Crest.Workflows.Designer/ (WASM App)
    OrchardCore.Crest.Workflows/ (Module, references Designer)
  apps/
    OrchardCore.Crest.Workflows.Web/ (Web App)
```
...
src/
  modules/
    OrchardCore.Crest.Workflows.Designer/ (RCL)
    OrchardCore.Crest.Workflows/ (Module, no Designer reference)
  apps/
    OrchardCore.Crest.Workflows.Web/ (Web App, references Blazor)
    OrchardCore.Crest.Workflows.Web.Blazor/ (WASM App, references Designer RCL)
```
...
2. **Reference Implementation:** OrchardCore.Crest.Workflows.Web.Blazor serves as example
3. **Simplified Module:** OrchardCore.Crest.Workflows no longer needs Designer reference
...
### Step 2: Reference OrchardCore.Crest.Workflows.Designer NuGet Package
```xml
<PackageReference Include="OrchardCore.Crest.Workflows.Designer" Version="3.6.0" />
```
...
### Step 5: Enable OrchardCore.Crest.Workflows Module
...
- OrchardCore.Crest.Workflows.Designer is packable (RCL)
- OrchardCore.Crest.Workflows.Web.Blazor is NOT packable (reference app only)
...
2. Reference OrchardCore.Crest.Workflows.Designer RCL
