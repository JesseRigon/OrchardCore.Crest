# Migration Guide: Upgrading to OrchardCore.Crest.Workflows 3.6.0+

## Overview

Starting with version 3.6.0, the OrchardCore.Crest.Workflows Designer architecture has changed. The Designer is now a Razor Class Library (RCL) instead of a bundled Blazor WebAssembly application.

## Breaking Changes

**The bundled WASM scenario has been removed.** You must now:
1. Create your own Blazor WebAssembly project
2. Reference the OrchardCore.Crest.Workflows.Designer RCL
...
The OrchardCore.Crest.Workflows module views should already be using the simplified script reference:
...
**Solution:** The OrchardCore.Crest.Workflows module views configure Blazor with `autostart="false"` and a `loadBootResource` callback to ensure all `_framework` resources load from the root path. If you've customized the views, make sure you include this configuration:
...
│       ├── wwwroot/
│       │   └── appsettings.json
│       └── YourProject.Blazor.csproj # References OrchardCore.Crest.Workflows.Designer
```

## Reference Implementation

See `OrchardCore.Crest.Workflows.Web.Blazor` in the source repository for a complete reference implementation.

## Need Help?

If you encounter issues during migration, please:
1. Check the reference implementation in `src/apps/OrchardCore.Crest.Workflows.Web.Blazor`
2. Review the README in the OrchardCore.Crest.Workflows.Web.Blazor project
