...
This module integrates [Elsa Workflows](https://github.com/elsa-workflows/elsa-core) with Orchard Core, as part of the [OrchardCore.Crest](https://github.com/JesseRigon/OrchardCore.Crest.git) module family.
...
- Run the sample CMS app:
  ...
  dotnet run --project src/apps/OrchardCore.Crest.Workflows.Web

  # Option B: run on https://localhost:8096 to match the Elsa BaseUrl in appsettings
  ASPNETCORE_URLS="https://localhost:8096" dotnet run --project src/apps/OrchardCore.Crest.Workflows.Web
  ```
...
- How it’s wired in this repo:
  - CMS app: `src/apps/OrchardCore.Crest.Workflows.Web`
  - Designer host (WASM): `src/apps/OrchardCore.Crest.Workflows.Web.BlazorWasm`
  - Designer module: `src/modules/OrchardCore.Crest.Workflows.Designer` (+ `OrchardCore.Crest.Workflows.Designer.BlazorWasm`)
...
- Customizing the designer:
  ...
  - Example: see `src/modules/OrchardCore.Crest.Workflows.Designer/ActivityIconProvider.cs` for per-activity icons/colors.
...
- Open `OrchardCore.Crest.Workflows.sln` to work with the Orchard Core modules and sample app.
