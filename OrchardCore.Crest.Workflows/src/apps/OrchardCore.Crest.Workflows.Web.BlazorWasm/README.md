# OrchardCore.Crest.Workflows.Web.Blazor
...
- **OrchardCore.Crest.Workflows.Designer** (RCL) - Contains all Blazor components and shared services
- **OrchardCore.Crest.Workflows.Designer.BlazorWasm** - Adds WebAssembly-specific services and extension methods
- **OrchardCore.Crest.Workflows.Web.Blazor** (WASM App) - Standalone Blazor WebAssembly application that references the WebAssembly wrapper
...
### 2. Add Reference to OrchardCore.Crest.Workflows.Designer.BlazorWasm

In your `YourProject.Blazor.csproj`:

```xml
<ItemGroup>
    <PackageReference Include="OrchardCore.Crest.Workflows.Designer.BlazorWasm" Version="3.6.0" />
</ItemGroup>
```
...
That's it! The `AddElsaDesigner()` extension method provided by `OrchardCore.Crest.Workflows.Designer.BlazorWasm` registers root components and configures all required services.
...
### 6. Enable OrchardCore.Crest.Workflows Module

Make sure to enable the `OrchardCore.Crest.Workflows` module in your OrchardCore application. The module's views already reference `/_framework/blazor.webassembly.js`, which will be served by your WASM project.

## How It Works

- The OrchardCore.Crest.Workflows module provides views that load the Blazor WASM runtime
...
The OrchardCore.Crest.Workflows.Designer package provides convenient extension methods:
...
Run the OrchardCore.Crest.Workflows.Web application alongside this WASM app:

```bash
# Terminal 1 - Run the OrchardCore web app
cd src/apps/OrchardCore.Crest.Workflows.Web
dotnet run
