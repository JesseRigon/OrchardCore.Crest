...
namespace OrchardCore.Crest.Workflows.Middleware;
...
public class ElsaStudioBlazorAssetsUrlRewriter(RequestDelegate next)
{
    public async Task Invoke(HttpContext context)
    {
            ...
            && path.Contains("/_content/", StringComparison.OrdinalIgnoreCase))
        {
            ...
            // We want to rewrite to /OrchardCore.Crest.Workflows/_content/Elsa.Studio.DomInterop/dom.entry.js
        ...
        else if (path.Contains("OrchardCore.Crest.Workflows", StringComparison.OrdinalIgnoreCase))
        {
            var index = path.IndexOf("/_content/", StringComparison.OrdinalIgnoreCase);
            if (index >= 0)
            {
                // Strip off the /OrchardCore.Crest.Workflows prefix.
                var newPath = path.Substring("/OrchardCore.Crest.Workflows".Length);
                context.Request.Path = newPath;
            }
        }
        else
