...
// Converted from the old admin-content-parts-page.js. Same assertions; also clears the
...
module.exports = async function run(page, ctx) {
  await page.goto(`${ctx.baseUrl}/Admin/ContentTypes/ListParts`, { waitUntil: 'networkidle' });
  ...
  const results = [{ name: 'renders-content-parts-list', pass: count >= 1, message: `${count} content parts` }];

  if (count >= 1) {
    const selectedName = await rows.first().locator('.crest-model-list__item-title').innerText();
    await rows.first().click();
    const detailShown = await page.locator('[data-testid="content-part-detail"]').waitFor({ timeout: 10000 }).then(() => true).catch(() => false);
    ...
    results.push({
      name: 'selecting-part-shows-detail',
      pass: detailShown && headingShown,
      message: `part=${selectedName} detail=${detailShown} heading=${headingShown}`,
    });
