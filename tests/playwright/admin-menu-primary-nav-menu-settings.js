...
async function savePrimaryNavMenuSettings(page, menuId, settings, action) {
...
async function main() {
  ...
  try {
    ...
    if (separatorSettledClass !== separatorChangedClass) {
      throw new Error('PrimaryNavMenu settings flyout reloaded and reverted an unsaved checkbox edit.');
    }
    ...
    await savePrimaryNavMenuSettings(page, menu.id, testSettings, 'save test primaryNavMenu settings');
    ...
    if (applied.hasToggle) {
      throw new Error('PrimaryNavMenu still rendered a collapse toggle after collapsible=false was saved.');
    }

    if (applied.expansionDuration !== '333ms') {
      throw new Error(`PrimaryNavMenu expansion duration was not applied. Found ${applied.expansionDuration}`);
    }

    if (applied.tier2BaseSize !== '13px') {
      throw new Error(`PrimaryNavMenu tier base size was not applied. Found ${applied.tier2BaseSize}`);
    }
    ...
    if (originalSettings) {
      ...
      const menu = state.menus?.find(menu => menu.isDefault) || state.menus?.find(menu => menu.id === 'default');
      if (menu?.id) {
        await savePrimaryNavMenuSettings(page, menu.id, originalSettings, 'restore primaryNavMenu settings');
      }
      await page.close();
    }
    await browser.close();
