...
async function main() {
  ...
  try {
    ...
    if (!menu?.id) {
      throw new Error('Default sidebar layout menu was not returned.');
    }

    menuId = menu.id;
    originalSettings = JSON.parse(JSON.stringify(menu.sidebarSettings));
    await postJson(page, `/api/crest/admin-menus/${encodeURIComponent(menuId)}/sidebar-settings`, {
      ...originalSettings,
      collapsible: true,
    ...
    const rootButton = sidebar.locator('.crest-panel-menu__item-link', { hasText: rootText }).first();
    await rootButton.waitFor({ timeout: 10000 });
    if (await rootButton.getAttribute('aria-expanded') !== 'true') {
      await rootButton.click();
    }

    const childButton = sidebar.locator('.crest-panel-menu__item-link', { hasText: childText }).first();
    await childButton.waitFor({ timeout: 10000 });
    if (await childButton.getAttribute('aria-expanded') !== 'true') {
      await childButton.click();
    }

    const trigger = sidebar.locator('.crest-panel-menu__item-link', { hasText: triggerText }).first();
    await trigger.waitFor({ timeout: 10000 });
    await trigger.scrollIntoViewIfNeeded();
    await trigger.hover();

    const popup = page.locator('.admin-menu-sidebar__flyout--detached').first();
    ...
    const anchoredPlacement = await page.evaluate(() => {
      const popupElement = document.querySelector('.admin-menu-sidebar__flyout--detached');
      return {
    ...
    const stillExpanded = await sidebar.evaluate(element => element.classList.contains('admin-menu-sidebar--expanded'));
    if (!stillExpanded) {
      throw new Error('Sidebar collapsed while the pointer was over the detached tier-4 popup.');
    }
    ...
    try {
      ...
      if (menuId) {
        ...
        if (originalSettings) {
          await postJson(page, `/api/crest/admin-menus/${encodeURIComponent(menuId)}/sidebar-settings`, originalSettings);
        }
      }
    } finally {
      await page.close();
