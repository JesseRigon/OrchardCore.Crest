...
async function main() {
  ...
  const out = 'chat/playwright/primaryNavMenu-visual-debug.png';
  await page.locator('.primary-nav-menu').screenshot({ path: out });

  const details = await page.locator('.primary-nav-menu .primary-nav-menu__item-content').evaluateAll(items => items.map(item => {
