...
async function main() {
  ...
  await page.locator('.primary-nav-menu').evaluate(primaryNavMenu => {
    const contentDefinition = Array.from(primaryNavMenu.querySelectorAll('.primary-nav-menu__item--level-1'))
      .find(item => (item.querySelector('.primary-nav-menu__item-content')?.textContent || '').trim() === 'Content Definition');
  ...
  const result = await page.locator('.primary-nav-menu').evaluate(primaryNavMenu => {
    const findItem = (level, text) => Array.from(primaryNavMenu.querySelectorAll(`.primary-nav-menu__item--level-${level}`))
      .find(item => (item.querySelector('.primary-nav-menu__item-content')?.textContent || '').trim() === text);

    const details = text => {
      const item = Array.from(primaryNavMenu.querySelectorAll('.primary-nav-menu__item--level-1'))
        .find(element => (element.querySelector('.primary-nav-menu__item-content')?.textContent || '').trim() === text);
      if (!item) {
        return null;
      }
    ...
    const children = levelTwoContainer ? Array.from(levelTwoContainer.querySelectorAll(':scope > .primary-nav-menu__item--level-2')).map(item => ({
