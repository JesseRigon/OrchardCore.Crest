...
`BlazingOrchard.Icons` owns icon infrastructure for Blazing Orchard:
...
This module may depend on `BlazingOrchard.Components` for shared Radzen-backed UI primitives.

`BlazingOrchard.Components` must not depend on this module. Admin/Site themes reference both projects when they need icon UI compiled into the WASM app.
