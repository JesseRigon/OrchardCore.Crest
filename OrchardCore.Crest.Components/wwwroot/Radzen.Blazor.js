...
window.Crest = {
  ...
  addMouseLeave: function (id, ref) {
     ...
     if (el) {
        ...
        Crest[id + 'mouseleave'] = handler;
        el.addEventListener('mouseleave', handler, false);
     }
  },
  removeContextMenu: function (id) {
      var el = document.getElementById(id);
      if (el && Crest[id + 'contextmenu']) {
          el.removeEventListener('contextmenu', Crest[id + 'contextmenu']);
      }
  },
  removeMouseEnter: function (id) {
      var el = document.getElementById(id);
      if (el && Crest[id + 'mouseenter']) {
          el.removeEventListener('mouseenter', Crest[id + 'mouseenter']);
      }
  },
  removeMouseLeave: function (id) {
      var el = document.getElementById(id);
      if (el && Crest[id + 'mouseleave']) {
          el.removeEventListener('mouseleave', Crest[id + 'mouseleave']);
      }
  },
  ...
  ganttInitDragResize: function (containerId, dotnetRef, timelineRangeMs, allowDrag, allowResize, rowHeightPx) {
    ...
    if (Crest[key]) {
      Crest[key].dotnetRef = dotnetRef;
      Crest[key].timelineRangeMs = timelineRangeMs;
      Crest[key].allowDrag = allowDrag;
      Crest[key].allowResize = allowResize;
      Crest[key].rowHeightPx = rowHeightPx || 36;
      return;
    }
  ...
  createMap: function (wrapper, ref, id, apiKey, mapId, zoom, center, markers, options, fitBoundsToMarkersOnUpdate, language) {
    var api = function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        Crest.loadGoogleMaps(defaultView, apiKey, resolve, reject, language);
      });
    };

    api().then(function (google) {
      Crest[id] = ref;
      Crest[id].google = google;

      Crest[id].instance = new google.maps.Map(wrapper, {
        center: center,
        zoom: zoom,
        mapId: mapId
      });

      Crest[id].instance.addListener('click', function (e) {
        try { Crest[id].invokeMethodAsync('RadzenGoogleMap.OnMapClick', {
          Position: {Lat: e.latLng.lat(), Lng: e.latLng.lng()}
        }); } catch { }
      });

      Crest.updateMap(id, apiKey, zoom, center, markers, options, fitBoundsToMarkersOnUpdate, language);
    });

    return {
      dispose: function () {
        if (Crest[id] && Crest[id].instance) {
          delete Crest[id].instance;
        }
      }
    };
  },
  updateMap: function (id, apiKey, zoom, center, markers, options, fitBoundsToMarkersOnUpdate, language) {
    ...
    api().then(function (google) {
        ...
        if (Crest[id] && Crest[id].instance) {
            if (Crest[id].instance.markers && Crest[id].instance.markers.length) {
                for (var i = 0; i < Crest[id].instance.markers.length; i++) {
                    Crest[id].instance.markers[i].setMap(null);
                }
            }

            if (markers) {
                Crest[id].instance.markers = [];
                ...
                markers.forEach(function (m) {
                    ...
                    marker.addListener('click', function (e) {
                        try { Crest[id].invokeMethodAsync('RadzenGoogleMap.OnMarkerClick', {
                            Title: marker.title,
                    ...
                    marker.setMap(Crest[id].instance);

                    Crest[id].instance.markers.push(marker);
            ...
            if (zoom) {
                Crest[id].instance.setZoom(zoom);
                }

            if (center) {
                Crest[id].instance.setCenter(center);
            }

            if (options) {
                Crest[id].instance.setOptions(options);
            }

            if (markers && fitBoundsToMarkersOnUpdate) {
                Crest[id].instance.fitBounds(markerBounds);
            }
        }
    });
  },
 focusSecurityCode: function (el) {
  ...
  createSecurityCode: function (id, ref, el, isNumber) {
      ...
      Crest[id].inputs = [...el.querySelectorAll('.rz-security-code-input')];

      var isAndroid = navigator.userAgent.match(/Android/i);

      Crest[id].paste = function (e) {
          if (e.clipboardData) {
              var value = e.clipboardData.getData('text');

              if (value) {
                  for (var i = 0; i < Math.min(value.length, Crest[id].inputs.length); i++) {
                      if (isNumber && isNaN(+value[i])) {
                          continue;
                      }
                      Crest[id].inputs[i].value = value[i];
                  }

                  var code = Crest[id].inputs.map(i => i.value).join('').trim();
                  hidden.value = code;

                  try { suppressDisposed(ref.invokeMethodAsync('RadzenSecurityCode.OnValueChange', code)); } catch { }

                  Crest[id].inputs[Crest[id].inputs.length - 1].focus();
              }

              e.preventDefault();
          }
      }
      Crest[id].keyPress = function (e) {
          ...
          var value = Crest[id].inputs.map(i => i.value).join('').trim();
          hidden.value = value;

          try { suppressDisposed(ref.invokeMethodAsync('RadzenSecurityCode.OnValueChange', value)); } catch { }

          var index = Crest[id].inputs.indexOf(e.currentTarget);
          if (index < Crest[id].inputs.length - 1) {
              Crest[id].inputs[index + 1].focus();
          }
      }

      Crest[id].keyDown = function (e) {
      ...
      for (var i = 0; i < Crest[id].inputs.length; i++) {
          Crest[id].inputs[i].addEventListener(isAndroid ? 'textInput' : 'keypress', Crest[id].keyPress);
          Crest[id].inputs[i].addEventListener('keydown', Crest[id].keyDown);
          Crest[id].inputs[i].addEventListener('paste', Crest[id].paste);
      }
      ...
      return {
          dispose: function () {
              ...
              if (Crest[id].keyPress && Crest[id].keyDown && Crest[id].paste) {
                  for (var i = 0; i < inputs.length; i++) {
                      inputs[i].removeEventListener(isAndroid ? 'textInput' : 'keypress', Crest[id].keyPress);
                      inputs[i].removeEventListener('keydown', Crest[id].keyDown);
                      inputs[i].removeEventListener('paste', Crest[id].paste);
                  }
                  delete Crest[id].keyPress;
                  delete Crest[id].keyDown;
                  delete Crest[id].paste;
              }
  ...
  createSlider: function (
    ...
    Crest[id] = {};
    Crest[id].mouseMoveHandler = function (e) {
      e.preventDefault();
      ...
      var percent = isVertical ? (parent.offsetHeight - handle.offsetTop - offsetY) / parent.offsetHeight
        : (Crest.isRTL(handle) ? parent.offsetWidth - handle.offsetLeft - offsetX : handle.offsetLeft + offsetX) / parent.offsetWidth;
    ...
    Crest[id].mouseDownHandler = function (e) {
      if (parent.classList.contains('rz-state-disabled')) return;

      document.addEventListener('mousemove', Crest[id].mouseMoveHandler);
      document.addEventListener('touchmove', Crest[id].mouseMoveHandler, {
        passive: false, capture: true
      });

      document.addEventListener('mouseup', Crest[id].mouseUpHandler);
      document.addEventListener('touchend', Crest[id].mouseUpHandler, {
        passive: true
      });
    ...
    Crest[id].mouseUpHandler = function (e) {
      slider.canChange = false;
      document.removeEventListener('mousemove', Crest[id].mouseMoveHandler);
      document.removeEventListener('touchmove', Crest[id].mouseMoveHandler, {
        passive: false, capture: true
      });
      document.removeEventListener('mouseup', Crest[id].mouseUpHandler);
      document.removeEventListener('touchend', Crest[id].mouseUpHandler, {
        passive: true
      });
    };

    parent.addEventListener('mousedown', Crest[id].mouseDownHandler);
    parent.addEventListener('touchstart', Crest[id].mouseDownHandler, {
      passive: true
    });
    ...
    return { dispose: function() {
      ...
      if (Crest[id].mouseMoveHandler) {
        document.removeEventListener('mousemove', Crest[id].mouseMoveHandler);
        document.removeEventListener('touchmove', Crest[id].mouseMoveHandler);
        delete Crest[id].mouseMoveHandler;
      }
      if (Crest[id].mouseUpHandler) {
        document.removeEventListener('mouseup', Crest[id].mouseUpHandler);
        document.removeEventListener('touchend', Crest[id].mouseUpHandler);
        delete Crest[id].mouseUpHandler;
      }
      if (Crest[id].mouseDownHandler) {
        parent.removeEventListener('mousedown', Crest[id].mouseDownHandler);
        parent.removeEventListener('touchstart', Crest[id].mouseDownHandler);
        delete Crest[id].mouseDownHandler;
      }
  ...
  selectListItem: function (input, ul, index) {
    ...
    if (
      ...
      Crest.updateActiveDescendant(ul, childNodes[ul.nextSelectedIndex], ul.nextSelectedIndex);
    } else {
      Crest.updateActiveDescendant(ul, null, -1);
    }
  },
  focusListItem: function (input, ul, isDown, startIndex) {
    ...
    if (
      ...
      childNodes[ul.nextSelectedIndex].classList.add('rz-state-highlight');
      Crest.scrollIntoViewIfNeeded(childNodes[ul.nextSelectedIndex]);
      Crest.updateActiveDescendant(ul, childNodes[ul.nextSelectedIndex], ul.nextSelectedIndex);
    } else {
      Crest.updateActiveDescendant(ul, null, -1);
    }
  ...
  focusVirtualListItem: function (ul, absoluteIndex, totalCount) {
    ...
    function highlightItem() {
      ...
      if (relativeIndex >= 0 && relativeIndex < lis.length) {
        ...
        Crest.updateActiveDescendant(ul, lis[relativeIndex], absoluteIndex);
      } else {
        Crest.updateActiveDescendant(ul, null, -1);
      }
    }
        ...
        if (
            ...
            if (!cell.classList.contains('rz-state-focused')) {
                ...
                if (!isVirtual && table.parentElement.scrollWidth > table.parentElement.clientWidth) {
                    Crest.scrollIntoViewIfNeeded(cell);
                }
            }
        ...
        if (table.nextSelectedIndex >= 0 &&
            ...
            if (!row.classList.contains('rz-state-focused')) {
                ...
                if (!isVirtual && table.parentElement.scrollHeight > table.parentElement.clientHeight) {
                    Crest.scrollIntoViewIfNeeded(row);
                }
            }
  ...
  uploadInputChange: function (e, url, auto, multiple, clear, parameterName, method = 'POST', stream = false) {
      if (auto) {
          Crest.upload(e.target, url, multiple, clear, parameterName, method, stream);
          e.target.value = '';
      } else {
          Crest.uploadChange(e.target);
      }
  },
  uploads: function (uploadComponent, id) {
    if (!Crest.uploadComponents) {
      Crest.uploadComponents = {};
    }
    Crest.uploadComponents[id] = uploadComponent;
  },
  uploadChange: function (fileInput) {
    ...
    var uploadComponent =
      Crest.uploadComponents && Crest.uploadComponents[fileInput.id];
    if (uploadComponent) {
  ...
  removeFileFromUpload: function (ref, name, id) {
    var uploadComponent = (Crest.uploadComponents && Crest.uploadComponents[ref]) ?? Crest.uploadComponents[id];
  ...
  upload: function (fileInput, url, multiple, clear, parameterName, method = 'POST', stream = false) {
    var uploadComponent = Crest.uploadComponents && Crest.uploadComponents[fileInput.id];
    if (!uploadComponent) { return; }
  ...
  getCulture: function () {
    var cultureCookie = Crest.getCookie('.AspNetCore.Culture');
  ...
  openContextMenu: function (x,y,id, instance, callback) {
    ...
    Crest.openPopup(null, id, false, null, x, y, instance, callback);
  ...
  openTooltip: function (target, id, delay, duration, position, closeTooltipOnDocumentClick, instance, callback, clientX, clientY) {
    ...
    if (target) {
        ...
        Crest[id + 'target'] = target;
    }

    if (delay) {
        Crest[id + 'delay'] = setTimeout(Crest.openPopup, delay, target, id, false, position, clientX, clientY, instance, callback, closeTooltipOnDocumentClick, false, disableSmartPosition);
    } else {
        Crest.openPopup(target, id, false, position, clientX, clientY, instance, callback, closeTooltipOnDocumentClick, false, disableSmartPosition);
    }

    if (duration) {
      Crest[id + 'duration'] = setTimeout(Crest.closePopup, duration, id, instance, callback);
    }
  },
  closeTooltip(id) {
    Crest.activeElement = null;

    var target = Crest[id + 'target'];
    if (target) {
        target.removeAttribute('aria-describedby');
        Crest[id + 'target'] = null;
    }

    Crest.closePopup(id);

    if (Crest[id + 'delay']) {
        clearTimeout(Crest[id + 'delay']);
    }

    if (Crest[id + 'duration']) {
        clearTimeout(Crest[id + 'duration']);
    }
  },
  createDatePicker(el, popupId, instance, callback) {
      ...
      var handler = function (e, condition) {
          if (condition) {
              Crest.togglePopup(e.currentTarget.parentNode, popupId, false, instance, callback, true, false);
          }
      };
  ...
  openPopup: function (parent, id, syncWidth, position, x, y, instance, callback, closeOnDocumentClick = true, autoFocusFirstElement = false, disableSmartPosition = false) {
    ...
    Crest.activeElement = document.activeElement;
    ...
    if (window.chrome) {
        ...
        if (closestFrozenCell) {
            Crest[id + 'FZL'] = { cell: closestFrozenCell, left: closestFrozenCell.style.left };
    ...
    Crest[id] = function (e) {
        var lastPopup = Crest.popups && Crest.popups[Crest.popups.length - 1];
        ...
        if (!/Android/i.test(navigator.userAgent) &&
            ...
            Crest.closePopup(currentPopup.id, currentPopup.instance, currentPopup.callback, e);
            return;
        }
    ...
    Crest.popups.push({ id, instance, callback, parent });
    ...
    document.removeEventListener('mousedown', Crest[id]);
    document.addEventListener('mousedown', Crest[id]);
    window.removeEventListener('resize', Crest[id]);
    window.addEventListener('resize', Crest[id]);
    ...
    while (p && p != document.body) {
        if (p.scrollWidth > p.clientWidth || p.scrollHeight > p.clientHeight) {
            p.removeEventListener('scroll', Crest.closeAllPopups);
            p.addEventListener('scroll', Crest.closeAllPopups);
        }
        p = p.parentElement;
    }

    if (!parent) {
        document.removeEventListener('contextmenu', Crest[id]);
        document.addEventListener('contextmenu', Crest[id]);
    }

    if (autoFocusFirstElement) {
        setTimeout(function () {
            popup.removeEventListener('keydown', Crest.focusTrap);
            popup.addEventListener('keydown', Crest.focusTrap);

            var focusable = Crest.getFocusableElements(popup);
    ...
    popup.__escapeHandler = function (e) {
        if (e.key === 'Escape' || e.key === 'Esc') {
            Crest.closePopup(id, instance, callback, e);
        }
        ...
        if (e.key === 'Tab' && !e.shiftKey) {
            ...
            if (timepicker) {
                var focusable = Crest.getFocusableElements(timepicker);
  ...
  closeAllPopups: function (e, id) {
    ...
    var popups = Crest.popups;
      for (var i = 0; i < popups.length; i++) {
        ...
        Crest.closePopup(p.id, p.instance, p.callback, e);
    }
    Crest.popups = [];
  },
  closePopup: function (id, instance, callback, e, preventFocusRestore) {
    var popup = document.getElementById(id);
    if (!popup) return;
    var popupInfo = (Crest.popups || []).find(function (p) { return p.id === id; });
    if (popupInfo && popupInfo.parent) {
      Crest.setPopupAriaExpanded(popupInfo.parent, id, false);
    ...
    if (popup) {
      ...
      if (window.chrome && Crest[id + 'FZL']) {
        Crest[id + 'FZL'].cell.style.left = Crest[id + 'FZL'].left;
        Crest[id + 'FZL'] = null;
      }
    ...
    Crest.popups = (Crest.popups || []).filter(function (obj) {
        return obj.id !== id;
    });

    if (!preventFocusRestore &&
        (Crest.activeElement && Crest.activeElement == document.activeElement ||
        Crest.activeElement && document.activeElement == document.body ||
        Crest.activeElement && popup && popup.contains(document.activeElement) ||
        Crest.activeElement && document.activeElement &&
        ...
        setTimeout(function () {
            if (e && e.target && e.target.tabIndex != -1) {
                Crest.activeElement = e.target;
            }
            if (Crest.activeElement) {
               Crest.activeElement.focus();
            }
            Crest.activeElement = null;
        }, 100);
    }
  ...
  togglePopup: function (parent, id, syncWidth, instance, callback, closeOnDocumentClick = true, autoFocusFirstElement = false) {
    ...
    if (popup.style.display == 'block') {
      Crest.closePopup(id, instance, callback);
    } else {
      Crest.openPopup(parent, id, syncWidth, null, null, null, instance, callback, closeOnDocumentClick, autoFocusFirstElement);
    }
  },
  ...
  focusFirstFocusableElement: function (el) {
      var focusable = Crest.getFocusableElements(el);
  ...
  openSideDialog: function (options) {
      setTimeout(function () {
          if (options.autoFocusFirstElement) {
              ...
              Crest.focusFirstFocusableElement(lastDialog);
          }
      }, 500);
  },
  ...
  openDialog: function (options, dialogService, dialog) {
    if (Crest.closeAllPopups) {
        Crest.closeAllPopups();
    }
    Crest.dialogService = dialogService;
    if (
    ...
    setTimeout(function () {
        ...
        if (lastDialog) {
            ...
            lastDialog.removeEventListener('keydown', Crest.focusTrap);
            lastDialog.addEventListener('keydown', Crest.focusTrap);
            ...
            if (options.draggable) {
                ...
                if (dialogTitle) {
                    ...
                    dialogTitle.addEventListener('mousedown', Crest[dialogTitle]);
                }
            }
    ...
    document.removeEventListener('keydown', Crest.closePopupOrDialog);
    if (options.closeDialogOnEsc) {
        document.addEventListener('keydown', Crest.closePopupOrDialog);
    }
  },
  closeDialog: function () {
    if (Crest.dialogResizer && typeof Crest.dialogResizer.disconnect === 'function') {
      Crest.dialogResizer.disconnect();
    }
    Crest.dialogResizer = null;
    ...
    if (lastDialog) {
        var dialogTitle = lastDialog.parentElement.querySelector('.rz-dialog-titlebar');
        if (dialogTitle) {
            dialogTitle.removeEventListener('mousedown', Crest[dialogTitle]);
            Crest[dialogTitle] = null;
            delete Crest[dialogTitle];
        }
    }

    if (dialogs.length <= 1) {
        document.removeEventListener('keydown', Crest.closePopupOrDialog);
        delete Crest.dialogService;
    }
  },
  ...
  focusTrap: function (e) {
    ...
    if (isTab) {
        var focusable = Crest.getFocusableElements(e.currentTarget);
        var firstFocusable = focusable[0];
  ...
  closePopupOrDialog: function (e) {
      ...
      if (isEscape && Crest.dialogService) {
          ...
          if (lastDialog && lastDialog.options && lastDialog.options.closeDialogOnEsc) {
              try {
                  Crest.dialogService.invokeMethodAsync('DialogService.TryClose', null).then(function(closed) {
                      if (closed && dialogs.length <= 1) {
                          document.removeEventListener('keydown', Crest.closePopupOrDialog);
                          delete Crest.dialogService;
                          var layout = document.querySelector('.rz-layout');
                          if (layout) {
                              layout.removeEventListener('keydown', Crest.disableKeydown);
                          }
                      }
                  });
              } catch (e) { }
  ...
  getNumericValue: function (arg) {
    ...
    return el ? Crest.getInputValue(el.children[0]) : null;
  },
  ...
  createRangeNavigator: function (ref, instance) {
    ...
    // Label state — set by Crest.updateRangeNavigatorLabels
  ...
  getSelectedCellBounds: function (ref, section) {
    ...
    selectedCells.forEach(function (selectedCell) {
      var position = Crest.getCellPosition(section, selectedCell, matrix);
  ...
  createEditor: function (ref, uploadUrl, paste, instance, shortcuts) {
    ...
    ref.keydownListener = function (e) {
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'c') {
        ...
        if (currentCell && ref.contains(currentCell)) {
          if (!currentCell.classList.contains('rz-html-editor-cell-selected')) {
            Crest.clearSelectedTableCells(ref);
            currentCell.classList.add('rz-html-editor-cell-selected');
          }
          e.preventDefault();
          Crest.copySelectedTableCells(ref);
          return;
        }
      }
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') {
        ...
        if (pasteCell && ref.contains(pasteCell) && ref.copiedCells && ref.copiedCells.matrix) {
          e.preventDefault();
          Crest.pasteSelectedTableCells(ref, pasteCell);
      ...
      if (e.key === 'Tab') {
        ...
        if (activeCell && ref.contains(activeCell)) {
          ...
          var matrix = Crest.getCellMatrix(section);
          var activePosition = Crest.getCellPosition(section, activeCell, matrix);
          ...
          if (!nextCell && !e.shiftKey && section && activeRow && activeRow.parentElement && activeRow.parentElement.tagName === 'TBODY') {
            ...
            if (table) {
              Crest.execTableCommand(ref, 'addRowAfter');
              matrix = Crest.getCellMatrix(section);
        ...
        if (e.target.matches('img')) {
          ...
          if (clickedCell && ref.contains(clickedCell)) {
            if (e.shiftKey && ref.lastSelectedTableCell) {
              Crest.selectTableCellRange(ref, ref.lastSelectedTableCell, clickedCell);
    ...
    ref.documentMouseMoveResizeListener = function (e) {
      if (ref.isResizingColumn && ref.resizeTargetCell && ref.isConnected) {
        ...
        var resizeMatrix = Crest.getCellMatrix(resizeSection);
        var resizePosition = Crest.getCellPosition(resizeSection, ref.resizeTargetCell, resizeMatrix);
        var resizeColumnIndex = resizePosition.columnIndex;
  ...
  startColumnReorder: function(id, gridId, gridRef) {
      ...
      Crest[id + 'cell'] = el.parentNode.parentNode;
      var visual = document.createElement("th");
      visual.className = Crest[id + 'cell'].className + ' rz-column-draggable';
      visual.style = Crest[id + 'cell'].style;
      visual.style.display = 'none';
      visual.style.position = 'absolute';
      visual.style.height = Crest[id + 'cell'].offsetHeight + 'px';
      visual.style.width = Crest[id + 'cell'].offsetWidth + 'px';
      visual.style.zIndex = 2000;
      visual.style.pointerEvents = 'none';
      visual.innerHTML = Crest[id + 'cell'].firstChild.outerHTML;
      visual.id = id + 'visual';
      document.body.appendChild(visual);

      var resizers = Crest[id + 'cell'].parentNode.querySelectorAll('.rz-column-resizer');
      for (let i = 0; i < resizers.length; i++) {
          resizers[i].style.display = 'none';
      }

      Crest[id + 'lastTouchX'] = null;
      Crest[id + 'lastTouchY'] = null;

      Crest[id + 'end'] = function (e) {
          var triggeredByTouch = e && (e.type === 'touchend' || e.type === 'touchcancel');
          var touchX = Crest[id + 'lastTouchX'];
          var touchY = Crest[id + 'lastTouchY'];
          ...
          grid.removeEventListener('mousemove', Crest[id + 'move']);
          grid.removeEventListener('touchmove', Crest[id + 'touchmove']);
          grid.removeEventListener('click', Crest[id + 'end']);
          document.removeEventListener('mouseup', Crest[id + 'end']);
          document.removeEventListener('touchend', Crest[id + 'end']);
          document.removeEventListener('touchcancel', Crest[id + 'end']);

          Crest[id + 'end'] = null;
          Crest[id + 'move'] = null;
          Crest[id + 'touchmove'] = null;
          Crest[id + 'lastTouchX'] = null;
          Crest[id + 'lastTouchY'] = null;
      ...
      grid.removeEventListener('click', Crest[id + 'end']);
      grid.addEventListener('click', Crest[id + 'end']);
      document.removeEventListener('mouseup', Crest[id + 'end']);
      document.addEventListener('mouseup', Crest[id + 'end']);
      document.removeEventListener('touchend', Crest[id + 'end']);
      document.addEventListener('touchend', Crest[id + 'end'], { passive: true });
      document.removeEventListener('touchcancel', Crest[id + 'end']);
      document.addEventListener('touchcancel', Crest[id + 'end'], { passive: true });

      Crest[id + 'move'] = function (e) {
          var el = document.getElementById(id + 'visual');
          if (el) {
      ...
      grid.removeEventListener('mousemove', Crest[id + 'move']);
      grid.addEventListener('mousemove', Crest[id + 'move']);

      Crest[id + 'touchmove'] = function (e) {
          if (e.touches && e.touches[0]) {
              Crest[id + 'lastTouchX'] = e.touches[0].clientX;
              Crest[id + 'lastTouchY'] = e.touches[0].clientY;
              Crest[id + 'move']({ clientX: e.touches[0].clientX, clientY: e.touches[0].clientY });
              if (e.cancelable) {
                  e.preventDefault();
              }
          }
      ...
      grid.removeEventListener('touchmove', Crest[id + 'touchmove']);
      grid.addEventListener('touchmove', Crest[id + 'touchmove'], { passive: false });
  },
  stopColumnResize: function (id, grid, columnIndex) {
    ...
    if (Crest[el]) {
        ...
        el.style.width = null;
        document.removeEventListener('mousemove', Crest[el].mouseMoveHandler);
        document.removeEventListener('mouseup', Crest[el].mouseUpHandler);
        document.removeEventListener('touchmove', Crest[el].touchMoveHandler)
        document.removeEventListener('touchend', Crest[el].mouseUpHandler);
        Crest[el] = null;
    }
  },
  ...
  startColumnResize: function(id, grid, columnIndex, clientX) {
      ...
      Crest[el] = {
          ...
          mouseMoveHandler: function (e) {
              if (Crest[el]) {
                  var widthFloat = (Crest[el].width - (Crest.isRTL(cell) ? -1 : 1) * (Crest[el].clientX - e.clientX));
                  ...
                  if (gridElement) {
                      Crest.updateFrozenColumnPositions(gridElement);
                  }
              }
          },
          touchMoveHandler: function (e) {
              if (e.targetTouches[0]) {
                  e.preventDefault();
                  Crest[el].mouseMoveHandler(e.targetTouches[0]);
              }
          }
      };
      el.style.width = "100%";
      document.addEventListener('mousemove', Crest[el].mouseMoveHandler);
      document.addEventListener('mouseup', Crest[el].mouseUpHandler);
      document.addEventListener('touchmove', Crest[el].touchMoveHandler, { passive: false })
      document.addEventListener('touchend', Crest[el].mouseUpHandler);
  },
      startSplitterResize: function(id,
        ...
        Crest[el] = {
            ...
            mouseUpHandler: function(e) {
                if (Crest[el]) {
                    ...
                    document.removeEventListener('pointerup', Crest[el].mouseUpHandler);
                    document.removeEventListener('pointercancel', Crest[el].mouseUpHandler);
                    document.removeEventListener('pointermove', Crest[el].mouseMoveHandler);
            ...
            mouseMoveHandler: function(e) {
                if (Crest[el]) {
                    ...
                    var spacePerc = Crest[el].panePerc + Crest[el].paneNextPerc;
                    var spaceLength = Crest[el].paneLength + Crest[el].paneNextLength;

                    var length = (Crest[el].paneLength -
                        (isHOrientation && Crest.isRTL(e.target) ? -1 : 1) * (Crest[el].clientPos - (isHOrientation ? e.clientX : e.clientY)));
                    ...
                    var perc = length / Crest[el].paneLength;
                    if (!isFinite(perc)) {
                        perc = 1;
                        Crest[el].panePerc = 0.1;
                        Crest[el].paneLength =isHOrientation
                            ? pane.getBoundingClientRect().width
                            : pane.getBoundingClientRect().height;
                    }

                    var newPerc =  Crest[el].panePerc * perc;
        ...
        const preventDefaultAndStopPropagation = (ev) => {
          ...
          document.addEventListener('pointerup', Crest[el].mouseUpHandler);
          document.addEventListener('pointercancel', Crest[el].mouseUpHandler);
          document.addEventListener('pointermove', Crest[el].mouseMoveHandler);
          el.addEventListener('touchmove', preventDefaultAndStopPropagation, { passive: false });
    },
    resizeSplitter(id, e) {
        var el = document.getElementById(id);
        if (el && Crest[el]) {
            Crest[el].mouseMoveHandler(e);
            Crest[el].mouseUpHandler(e);
        }
    },
    openWaiting: function() {
        ...
        if (Crest.WaitingIntervalId != null) {
            clearInterval(Crest.WaitingIntervalId);
        }
        ...
        setTimeout(function() {
                ...
                Crest.WaitingIntervalId = setInterval(function() {
    ...
    openChartTooltip: function (chart, x, y, id, instance, callback) {
        ...
        Crest.openPopup(chart, id, false, null, x, y, instance, callback, true, false, false);
...
Crest.registerFabMenu = function(element, dotnet){
  if(!element) return;
...
Crest.unregisterFabMenu = function(element){
  if(!element) return;
...
Crest.chatIsNearBottom = function(container, threshold) {
  if (!container) return true;
  return container.scrollHeight - container.scrollTop - container.clientHeight <= (threshold || 50);
};
Crest.chatScrollToBottom = function(container) {
  if (!container) return;
  container.scrollTop = container.scrollHeight;
};
Crest.noop = function() {};
Crest.getCookies = function() {
  return document.cookie;
};
Crest.setCookie = function(cookie) {
  document.cookie = cookie;
};
Crest.scrollToTop = function(elementId) {
  try { var el = document.getElementById(elementId); if (el) el.scrollTop = 0; } catch(e) {}
};
Crest.scrollIntoView = function(selector) {
  var el = document.querySelector(selector);
  if (el) el.scrollIntoView();
};
Crest.delayedFocus = function(elementId, delay) {
  setTimeout(function() { var el = document.getElementById(elementId); if (el) el.focus(); }, delay || 200);
};
Crest.invokeRadzenCallback = function(key) {
  var fn = Crest[key];
  if (typeof fn === 'function') fn();
};
Crest.chatScrollAfterRender = function(selector, delay) {
  setTimeout(function() {
    var container = document.querySelector(selector);
    if (container) container.scrollTop = container.scrollHeight;
  }, delay || 100);
};
Crest.createAutoComplete = function(el, popupId, openOnFocus) {
  if (!el) return null;
  ...
  if (!input) return null;
  function onInput() { Crest.openPopup(input.parentNode, popupId, true); }
  function onFocus() { Crest.openPopup(input.parentNode, popupId, true); }
  function clearActive() { Crest.activeElement = null; }
...
Crest.createDataGrid = function(el) {
  if (!el) return null;
  function onClick(e) {
    ...
    if (btn && el.contains(btn) && label && label.contains(btn) && btn.getAttribute('aria-controls')) {
      Crest.togglePopup(btn.parentNode, btn.getAttribute('aria-controls'));
    }
  }
  function onMousedown(e) {
    var btn = e.target.closest('.rz-apply-filter');
    if (btn) { Crest.blur(btn, e); }
  }
...
Crest.createDropDown = function(el) {
  if (!el) return null;
  function onMousedown() { Crest.activeElement = null; }
...
Crest.createFileInput = function(el) {
  if (!el) return null;
  ...
  function onChange(e) { Crest.uploadInputChange(e, null, false, false, true); }
...
Crest.createMask = function(el, id, mask, pattern, charPattern) {
  if (!el) return null;
  function onInput() { Crest.mask(id, mask, pattern, charPattern); }
  el.addEventListener('input', onInput);
  return { dispose: function() { el.removeEventListener('input', onInput); }};
};
Crest.createNumeric = function(el, isInteger, separator, min, max, isNullable) {
  if (!el) return null;
  var input = el.querySelector('.rz-numeric-input');
  if (!input) return null;
  function onKeypress(e) { Crest.numericKeyPress(e, isInteger, separator); }
  function onBlur(e) { Crest.numericOnInput(e, min, max, isNullable); }
  function onPaste(e) { Crest.numericOnPaste(e, min, max); }
...
Crest.createMenu = function(el, clickToOpen) {
  ...
  function onClick(e) {
    ...
    if (clickToOpen || hasChildren) {
      Crest.toggleMenuItem(item);
    } else {
      Crest.toggleMenuItem(item, e, false);
    }
  }
  el.addEventListener('click', onClick);
  var hoverItems = [];
  if (!clickToOpen) {
    el.querySelectorAll('.rz-navigation-item').forEach(function(navItem) {
      ...
      function onEnter(e) { Crest.toggleMenuItem(li, e, true, false); }
      function onLeave(e) { Crest.toggleMenuItem(li, e, false, false); }
...
Crest.createUpload = function(el, url, auto, multiple, parameterName, method, stream) {
  if (!el) return null;
  var choose = el.querySelector('.rz-fileupload-choose');
  var fileInput = el.querySelector('input[type=file]');
  function onKeydown(e) {
  ...
  function onStopKeydown(e) { e.stopPropagation(); }
  function onChange(e) {
    Crest.uploadInputChange(e, url, auto, multiple, true, parameterName, method, stream);
  }
...
Crest.beginRangePickKeyHandler = function (dotNetRef) {
  Crest.endRangePickKeyHandler();
  Crest.rangePickKeyHandler = function (e) {
    if (e.key === 'Escape') {
      e.preventDefault();
      e.stopPropagation();
  ...
  document.addEventListener('keydown', Crest.rangePickKeyHandler, true);
};

Crest.endRangePickKeyHandler = function () {
  if (Crest.rangePickKeyHandler) {
    document.removeEventListener('keydown', Crest.rangePickKeyHandler, true);
    Crest.rangePickKeyHandler = null;
  }
};
...
Crest.createSheetEditor = (element, value, autoFocus, dotNetRef) => new SheetEditor(element, value, autoFocus, dotNetRef);
Crest.createSpreadsheet = (element, dotNetRef, shortcuts) => new Spreadsheet(element, dotNetRef, shortcuts);
Crest.createVirtualItemContainer = (scrollable, content, ref) => {
...
Crest.scrollElementTo = (scrollable, left, top) => {
  scrollable.scrollTo({ top: top, left: left });
};
Crest.createFormField = function(el) {
...
Crest.createSignaturePad = function(element, ref, strokeColor, strokeWidth, disabled, initialValue) {
