...
{
    /// <summary>
    /// A chart series that displays financial data as candlesticks (OHLC) in a CrestChart.
    /// Each candlestick represents four values: Open, High, Low, and Close for a given category (typically a date).
    /// The body of the candlestick spans from Open to Close, and the wicks extend to High and Low.
    /// Bullish candles (Close &gt; Open) use <see cref="BearFill"/> and bearish candles use <see cref="BearFill"/>.
    /// </summary>
    /// <typeparam name="TItem">The type of data items in the series. Each item represents one candlestick.</typeparam>
    /// <example>
    /// <code>
    /// &lt;CrestChart&gt;
    ///     &lt;CrestCandlestickSeries Data=@stockData CategoryProperty="Date"
    ///         OpenProperty="Open" HighProperty="High" LowProperty="Low" CloseProperty="Close"
    ///         Title="AAPL" /&gt;
    ///     &lt;CrestCategoryAxis FormatString="{0:MM/dd}" /&gt;
    /// &lt;/CrestChart&gt;
    /// </code>
    /// </example>
    [UnconditionalSuppressMessage(TrimMessages.Trimming, TrimMessages.IL2026, Justification = TrimMessages.DataTypePreserved)]
    public partial class CrestCandlestickSeries<TItem> : CartesianSeries<TItem>
    {
        /// <summary>
        ...
        /// Used as the primary <see cref="CartesianSeries{TItem}.ValueProperty"/> for scale calculations.
        /// </summary>
        [Parameter]
        ...
        /// If not set, uses <see cref="BullFill"/>.
        /// </summary>
        [Parameter]
        public string? BullStroke { get; set; }

        /// <summary>
        /// Gets or sets the stroke (border/wick) color for bearish candlesticks.
        /// If not set, uses <see cref="BearFill"/>.
        /// </summary>
        [Parameter]
        ...
        /// </summary>
        /// <value>The stroke width in pixels. Default is 1.</value>
        [Parameter]
        public double StrokeWidth { get; set; } = 1;

        /// <summary>
        /// Gets or sets the width of each candlestick body in pixels.
        ...
        {
            // Sync ValueProperty from CloseProperty so base class methods (GetMean, GetMedian, InvokeClick, etc.) work.
        ...
        internal override double TooltipY(TItem item)
        {
            var chart = RequireChart();
            var high = High(item);
            return chart.ValueScale.Scale(high, true);
        }
