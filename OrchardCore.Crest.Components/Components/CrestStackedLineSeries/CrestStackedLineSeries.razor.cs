...
{
    /// <summary>
    /// Renders stacked line series in <see cref="CrestChart" />.
    /// </summary>
    /// <typeparam name="TItem">The type of data items in the series.</typeparam>
    public partial class CrestStackedLineSeries<TItem> : CartesianSeries<TItem>, IChartStackedAreaSeries
    {
        /// <inheritdoc />
        ...
        private IList<IChartSeries> LineSeries => Chart?.Series?.Where(series => series is IChartStackedAreaSeries).Cast<IChartSeries>().ToList() ?? new List<IChartSeries>();
        private IList<IChartSeries> VisibleLineSeries => LineSeries.Where(series => series.Visible).ToList();
        private IList<IChartStackedAreaSeries> StackedLineSeries => VisibleLineSeries.Cast<IChartStackedAreaSeries>().ToList();
        ...
        public override ScaleBase TransformValueScale(ScaleBase scale)
        {
            ...
            if (Items.Any())
            {
                var stackedLineSeries = StackedLineSeries;

                if (stackedLineSeries.Count > 0)
                {
                    var count = stackedLineSeries.Max(series => series.Count);
                    var sums = Enumerable.Range(0, count).Select(i => stackedLineSeries.Sum(series => series.ValueAt(i)));
                    var max = sums.Max();
