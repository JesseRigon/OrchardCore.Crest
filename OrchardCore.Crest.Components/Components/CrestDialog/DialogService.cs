...
namespace Crest.Components.Primitives
{
    ...
    /// Should be added as scoped service in the application services and CrestDialog should be added in application main layout.
    /// </summary>
    /// <example>
    /// <code>
    /// @inject DialogService DialogService
    /// &lt;CrestButton Text="Show dialog with inline Blazor content" Click=@ShowInlineDialog /&gt;
    /// @code {
    ...
    ///          &lt;div class="row"&gt;
    ///              &lt;div class="col-md-12"&gt;
    ///                  &lt;CrestButton Text="Ok" Click="() =&gt; ds.Close(true)" Style="margin-bottom: 10px; width: 150px" /&gt;
    ///                  &lt;CrestButton Text="Cancel" Click="() =&gt; ds.Close(false)" ButtonStyle="ButtonStyle.Base"  Style="margin-bottom: 10px; width: 150px"/&gt;
    ///                  &lt;CrestButton Text="Refresh" Click="(() =&gt; { orderID = 10249; ds.Refresh(); })" ButtonStyle="ButtonStyle.Info"  Style="margin-bottom: 10px; width: 150px"/&gt;
    ///                  Order ID: @orderID
    ///              &lt;/div&gt;
    ///          &lt;/div&gt;
    ///      &lt;/div&gt;);
    ...
    public class DialogService : IDisposable
    {
        ...
        {
            message ??= Localize(nameof(CrestStrings.Dialog_ConfirmMessage));
            title ??= Localize(nameof(CrestStrings.Dialog_ConfirmTitle));

            // Validate and set default values for the dialog options
            options ??= new();
            options.OkButtonText = !String.IsNullOrEmpty(options.OkButtonText) ? options.OkButtonText : Localize(nameof(CrestStrings.Dialog_OkText));
            options.CancelButtonText = !String.IsNullOrEmpty(options.CancelButtonText) ? options.CancelButtonText : Localize(nameof(CrestStrings.Dialog_CancelText));
        ...
        {
            title ??= Localize(nameof(CrestStrings.Dialog_AlertTitle));
    ...
    public class SideDialogOptions : DialogOptionsBase
    {
        ...
        {
            get => resizeBarTitle ?? Localizer.Default.Get(nameof(CrestStrings.Dialog_ResizeBarTitle), System.Globalization.CultureInfo.CurrentUICulture);
            set
            {
        ...
        {
            get => resizeBarAriaLabel ?? Localizer.Default.Get(nameof(CrestStrings.Dialog_ResizeBarAriaLabel), System.Globalization.CultureInfo.CurrentUICulture);
            set
            {
