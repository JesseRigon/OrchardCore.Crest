...
namespace Crest.Components.Primitives
{
    ...
    /// CrestCheckBoxList displays multiple checkboxes with configurable layout, orientation, and data binding, binding to a collection of selected values.
    ...
    /// <code>
    /// &lt;CrestCheckBoxList @bind-Value=@selectedOptions TValue="string"&gt;
    ///     &lt;Items&gt;
    ///         &lt;CrestCheckBoxListItem Text="Email notifications" Value="email" /&gt;
    ///         &lt;CrestCheckBoxListItem Text="SMS notifications" Value="sms" /&gt;
    ///         &lt;CrestCheckBoxListItem Text="Push notifications" Value="push" /&gt;
    ///     &lt;/Items&gt;
    /// &lt;/CrestCheckBoxList&gt;
    /// @code {
    ...
    /// <code>
    /// &lt;CrestCheckBoxList @bind-Value=@selectedIds TValue="int" Data=@categories 
    ...
    public partial class CrestCheckBoxList<TValue> : FormComponent<IEnumerable<TValue>>, ICrestCheckBoxList
    {
        ...
        void UpdateAllItems()
        {
            ...
            {
                var item = new CrestCheckBoxListItem<TValue>();
        ...
        async Task SelectAll(bool? value)
        {
            ...
            if (value == true)
            {
                Value = allItems.Where(i => !i.Disabled).Select(i => i.Value is TValue typedValue ? typedValue : default!).ToList();
            }
            else if (value == false)
            {
        ...
        protected async System.Threading.Tasks.Task SelectItem(ICrestCheckBoxListItem item)
        {
            ...
            {
                if (!selectedValues.Remove(typedValue))
                {
        ...
        string? GetActiveDescendantId()
        {
            ...
            return allItems.ElementAtOrDefault(focusedIndex)?.GetItemId();
        }
        void OnFocus()
        {
