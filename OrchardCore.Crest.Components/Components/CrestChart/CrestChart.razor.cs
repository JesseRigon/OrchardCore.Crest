...
namespace Crest.Components.Primitives
{
    ...
    /// CrestChart supports multiple series, customizable axes, legends, tooltips, data labels, markers, and interactive features.
    /// Container for one or more chart series components. Each series (CrestLineSeries, CrestColumnSeries, CrestAreaSeries, etc.) defines how data is visualized.
    ...
    /// Series are defined as child components within the CrestChart. Configure axes using CrestCategoryAxis and CrestValueAxis, customize the legend with CrestLegend, and add tooltips with CrestChartTooltipOptions.
    /// </summary>
    /// <example>
    /// Basic column chart:
    /// <code>
    ///   &lt;CrestChart&gt;
    ///       &lt;CrestColumnSeries Data=@revenue CategoryProperty="Quarter" Title="Revenue" ValueProperty="Revenue" /&gt;
    ///   &lt;/CrestChart&gt;
    ///   @code {
    ///       class DataItem
    ///       {
    ...
    /// <code>
    /// &lt;CrestChart&gt;
    ///     &lt;CrestLineSeries Data=@sales2023 CategoryProperty="Month" ValueProperty="Amount" Title="2023 Sales" /&gt;
    ///     &lt;CrestLineSeries Data=@sales2024 CategoryProperty="Month" ValueProperty="Amount" Title="2024 Sales" /&gt;
    ///     &lt;CrestLegend Position="LegendPosition.Bottom" /&gt;
    ///     &lt;CrestCategoryAxis Formatter="@(value =&gt; value.ToString())" /&gt;
    ///     &lt;CrestValueAxis Formatter="@(value =&gt; value.ToString("C"))" /&gt;
    /// &lt;/CrestChart&gt;
    /// </code>
    /// </example>
    public partial class CrestChart : CrestComponent
    {
        /// <summary>
        ...
        private static readonly Dictionary<string, List<CrestChart>> syncGroups = new Dictionary<string, List<CrestChart>>();
        ...
        internal CrestColumnOptions ColumnOptions { get; set; } = new CrestColumnOptions();
        internal CrestBarOptions BarOptions { get; set; } = new CrestBarOptions();
        internal CrestLegend Legend { get; set; } = new CrestLegend();
        internal CrestCategoryAxis CategoryAxis { get; set; } = new CrestCategoryAxis();
        internal CrestValueAxis ValueAxis { get; set; } = new CrestValueAxis();
        internal Dictionary<string, CrestValueAxis> AdditionalValueAxes { get; set; } = new Dictionary<string, CrestValueAxis>();
        internal CrestChartTooltipOptions Tooltip { get; set; } = new CrestChartTooltipOptions();
        ...
        internal void AddValueAxis(string name, CrestValueAxis axis)
        {
        ...
        internal CrestValueAxis GetValueAxis(string? name)
        {
            if (string.IsNullOrEmpty(name))
            {
                return ValueAxis;
        ...
        // Walk the base chain so user subclasses (e.g. class MyPie : CrestPieSeries<MyItem>) are matched too.
        private static bool IsPolarSeries(IChartSeries series)
        {
            for (var type = series.GetType(); type != null; type = type.BaseType)
            {
                ...
                if (definition == typeof(CrestPieSeries<>)
                    || definition == typeof(CrestDonutSeries<>)
                    || definition == typeof(CrestFunnelSeries<>)
                    || definition == typeof(CrestPyramidSeries<>))
                {
                    return true;
                }
            }
        ...
        /// <summary>
        /// Invoked via interop when the CrestChart resizes. Display the series with the new dimensions.
        /// </summary>
        ...
        /// <summary>
        /// Invoked via interop when the user moves the mouse over the CrestChart. Displays the tooltip.
        /// </summary>
        ...
        /// <summary>
        /// Invoked via interop when the user clicks the CrestChart. Raises the <see cref="SeriesClick" /> handler.
        /// </summary>
        /// <param name="x">The x.</param>
        /// <param name="y">The y.</param>
        [JSInvokable]
        ...
        internal CrestChartRangeNavigator? RangeNavigator { get; set; }
        ...
        private static void RenderCrosshairLine(Microsoft.AspNetCore.Components.Rendering.RenderTreeBuilder builder, int seqBase,
            CrestAxisCrosshair config, double x1, double x2, double y1, double y2)
        {
