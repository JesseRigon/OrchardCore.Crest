...
{
    /// <summary>
    /// CrestDropZoneItem component.
    /// </summary>
    public partial class CrestDropZoneItem<TItem> : CrestComponent
    {
        [CascadingParameter]
        TItem Item { get; set; } = default!;

        [CascadingParameter]
        CrestDropZone<TItem> Zone { get; set; } = default!;

        [CascadingParameter]
        CrestDropZoneContainer<TItem> Container { get; set; } = default!;

        void EnsurePayload(DragEventArgs? args = null)
        {
            Container.Payload = new CrestDropZoneItemEventArgs<TItem>()
            {
                FromZone = Zone,
                Item = Item,
