...
/// <summary>
/// Exposes some of the methods of the CrestSpreadsheet component.
/// </summary>
public interface ISpreadsheet
{
    ...
    /// <see cref="CrestSpreadsheet"/>. Tools should use this to decide whether
...
/// </summary>
public partial class CrestSpreadsheet : CrestComponent, IAsyncDisposable, ISpreadsheet
{
    ...
    /// toolbar's <see cref="CrestTabs.Tabs"/> slot — each child should be a
    /// <see cref="CrestTabsItem"/>. Add
    /// <see cref="CrestSpreadsheetTableDesignToolset"/> to keep the
    ...
    private async Task OnSheetAction(CrestSplitButtonItem? item, Worksheet sheet)
    {
        if (item is null)
        {
            return;
    ...
    private async Task OnRenameSheetAsync(Worksheet sheet)
    {
        ...
        var name = await OpenDialogAsync<Spreadsheet.RenameSheetDialog>(Localize(nameof(CrestStrings.Spreadsheet_RenameSheetTitle)),
    ...
    private async Task OnCellMenuCustomFilterAsync()
    {
        ...
        if (Worksheet != null)
        {
            ...
            var result = await OpenDialogAsync<FilterDialog>(Localize(nameof(CrestStrings.Spreadsheet_CustomFilterTitle)), parameters, new DialogOptions
            {
                Width = "600px",
            });
    ...
    private async Task OnCellMenuTop10FilterAsync()
    {
        ...
        var result = await OpenDialogAsync<Spreadsheet.Top10FilterDialog>(
            Localize(nameof(CrestStrings.Spreadsheet_Top10Filter)),
            null,
    ...
    private async Task OnFormatCellsAsync()
    {
            ...
            Worksheet.Cells.TryGet(Worksheet.Selection.Cell.Row, Worksheet.Selection.Cell.Column, out var cell))
        {
            ...
            var result = await OpenDialogAsync<FormatCellsDialog>(
                Localize(nameof(CrestStrings.Spreadsheet_FormatCellsTitle)), parameters, new DialogOptions { Width = "600px" });
                ...
                if (!valid && Editor!.Cell is not null)
                {
                    ...
                    switch (errorStyle)
                    {
                        case DataValidationErrorStyle.Information:
                            await DialogService.Alert(error, Localize(nameof(CrestStrings.Spreadsheet_InformationTitle)));
                        ...
                        case DataValidationErrorStyle.Warning:
                            var confirmed = await DialogService.Confirm(
                                error + Environment.NewLine + Environment.NewLine + Localize(nameof(CrestStrings.Spreadsheet_DoYouWantToContinue)),
                                Localize(nameof(CrestStrings.Spreadsheet_WarningTitle)));
                        ...
                        default: // Stop
                            await DialogService.Alert(error, Localize(nameof(CrestStrings.Spreadsheet_InvalidValueTitle)));
                            Editor!.Cell.ClearValidationErrors();
                            command.Unexecute();
                            Editor!.Cancel();
                            result = false;
    ...
    private void AnnounceDrawing()
    {
        ...
        if (Worksheet.SelectedImage is SheetImage image)
        {
            accessibility.AnnounceAction(
                $"{Localize(nameof(CrestStrings.Spreadsheet_A11yImageSelected))}, {new CellRef(image.From.Row, image.From.Column)}");
        }
        else if (Worksheet.SelectedChart is SheetChart chart)
        {
            accessibility.AnnounceAction(
                $"{Localize(nameof(CrestStrings.Spreadsheet_A11yChartSelected))}, {new CellRef(chart.From.Row, chart.From.Column)}");
        }
    }
    ...
    private async Task OpenDrawingSizeDialogAsync()
    {
        ...
        var parameters = new Dictionary<string, object?>
        {
            ...
            { nameof(Spreadsheet.DrawingSizeDialog.WidthLabel), Localize(nameof(CrestStrings.Spreadsheet_SizeWidth)) },
            { nameof(Spreadsheet.DrawingSizeDialog.HeightLabel), Localize(nameof(CrestStrings.Spreadsheet_SizeHeight)) },
            { nameof(Spreadsheet.DrawingSizeDialog.OkText), Localize(nameof(CrestStrings.Spreadsheet_OK)) },
            { nameof(Spreadsheet.DrawingSizeDialog.CancelText), Localize(nameof(CrestStrings.Spreadsheet_Cancel)) },
        };

        var result = await OpenDialogAsync<Spreadsheet.DrawingSizeDialog>(
            Localize(nameof(CrestStrings.Spreadsheet_SizeTitle)), parameters, new DialogOptions { Width = "340px" });
    ...
    {
        SortCommand or MultiKeySortCommand => Localize(nameof(CrestStrings.Spreadsheet_A11ySorted)),
        FilterCommand or TableFilterCommand or SheetAutoFilterCommand or RemoveFilterCommand
            => Localize(nameof(CrestStrings.Spreadsheet_A11yFilterApplied)),
        InsertRowCommand => Localize(nameof(CrestStrings.Spreadsheet_A11yRowInserted)),
        InsertColumnCommand => Localize(nameof(CrestStrings.Spreadsheet_A11yColumnInserted)),
        DeleteRowsCommand => Localize(nameof(CrestStrings.Spreadsheet_A11yRowDeleted)),
        DeleteColumnsCommand => Localize(nameof(CrestStrings.Spreadsheet_A11yColumnDeleted)),
        _ => null
    };
    ...
    [
        new("Arrow keys", nameof(CrestStrings.Spreadsheet_HelpMove), ["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"]),
        new("Tab / Shift+Tab", nameof(CrestStrings.Spreadsheet_HelpNextCell), ["Tab", "Shift+Tab"]),
        new("Enter / Shift+Enter", nameof(CrestStrings.Spreadsheet_HelpConfirmMove), ["Enter", "Shift+Enter"]),
        new("F6 / Shift+F6", nameof(CrestStrings.Spreadsheet_HelpRegions), ["F6", "Shift+F6"]),
        new("F2", nameof(CrestStrings.Spreadsheet_HelpEdit), ["F2"]),
        new("Escape", nameof(CrestStrings.Spreadsheet_HelpCancel), ["Escape"]),
        new("Shift+Arrow keys", nameof(CrestStrings.Spreadsheet_HelpExtend), ["Shift+ArrowUp", "Shift+ArrowDown", "Shift+ArrowLeft", "Shift+ArrowRight"]),
        new("Ctrl+Arrow keys", nameof(CrestStrings.Spreadsheet_HelpEdge), ["Ctrl+ArrowUp", "Ctrl+ArrowDown", "Ctrl+ArrowLeft", "Ctrl+ArrowRight"]),
        new("Ctrl+Shift+Arrow keys", nameof(CrestStrings.Spreadsheet_HelpExtendEdge), ["Ctrl+Shift+ArrowUp", "Ctrl+Shift+ArrowDown", "Ctrl+Shift+ArrowLeft", "Ctrl+Shift+ArrowRight"]),
        new("Home / Ctrl+Home", nameof(CrestStrings.Spreadsheet_HelpRowStart), ["Home", "Ctrl+Home"]),
        new("End / Ctrl+End", nameof(CrestStrings.Spreadsheet_HelpLastCell), ["End", "Ctrl+End"]),
        new("Shift+Home / Shift+End", nameof(CrestStrings.Spreadsheet_HelpExtendRow), ["Shift+Home", "Shift+End"]),
        new("Ctrl+Shift+Home / Ctrl+Shift+End", nameof(CrestStrings.Spreadsheet_HelpExtendSheet), ["Ctrl+Shift+Home", "Ctrl+Shift+End"]),
        new("Page Up / Page Down", nameof(CrestStrings.Spreadsheet_HelpPage), ["PageUp", "PageDown"]),
        new("Shift+Page Up / Shift+Page Down", nameof(CrestStrings.Spreadsheet_HelpExtendPage), ["Shift+PageUp", "Shift+PageDown"]),
        new("Ctrl+A", nameof(CrestStrings.Spreadsheet_HelpSelectAll), ["Ctrl+A"]),
        new("Ctrl+Space", nameof(CrestStrings.Spreadsheet_HelpSelectColumn), ["Ctrl+Space"]),
        new("Shift+Space", nameof(CrestStrings.Spreadsheet_HelpSelectRow), ["Shift+Space"]),
        new("Ctrl+C / Ctrl+X / Ctrl+V", nameof(CrestStrings.Spreadsheet_HelpClipboard), ["Ctrl+C", "Ctrl+X"]),
        new("Ctrl+Z / Ctrl+Y", nameof(CrestStrings.Spreadsheet_HelpUndoRedo), ["Ctrl+Z", "Ctrl+Shift+Z", "Ctrl+Y"]),
        new("Ctrl+D / Ctrl+R", nameof(CrestStrings.Spreadsheet_HelpFill), ["Ctrl+D", "Ctrl+R"]),
        new("Delete", nameof(CrestStrings.Spreadsheet_HelpClear), ["Delete", "Backspace"]),
        new("Shift+F10", nameof(CrestStrings.Spreadsheet_HelpContextMenu), ["Shift+F10", "ContextMenu"]),
        new("Ctrl+Alt+5", nameof(CrestStrings.Spreadsheet_HelpDrawings), ["Ctrl+Alt+5"]),
        new("Alt+/", nameof(CrestStrings.Spreadsheet_HelpShowShortcuts), ["Alt+Slash"]),
    ];
    ...
    private async Task OpenShortcutsHelpAsync()
    {
        ...
        var parameters = new Dictionary<string, object?>
        {
            ...
            { nameof(Spreadsheet.SpreadsheetShortcutsDialog.ShortcutColumn), Localize(nameof(CrestStrings.Spreadsheet_HelpShortcutColumn)) },
            { nameof(Spreadsheet.SpreadsheetShortcutsDialog.ActionColumn), Localize(nameof(CrestStrings.Spreadsheet_HelpActionColumn)) },
            { nameof(Spreadsheet.SpreadsheetShortcutsDialog.FilterPlaceholder), Localize(nameof(CrestStrings.Spreadsheet_HelpFilterPlaceholder)) },
            { nameof(Spreadsheet.SpreadsheetShortcutsDialog.NoResultsText), Localize(nameof(CrestStrings.Spreadsheet_HelpNoResults)) },
        };

        await OpenDialogAsync<Spreadsheet.SpreadsheetShortcutsDialog>(
            Localize(nameof(CrestStrings.Spreadsheet_HelpTitle)), parameters, new DialogOptions { Width = "600px" });
    }
    ...
    public async Task OnCellContextMenuAsync(CellEventArgs args)
    {
        ...
        if (canCutPaste)
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_Cut)), Value = "cut", Icon = "content_cut" });
        }
        if (IsFeatureAllowed(SpreadsheetFeature.Clipboard))
        {
            if (IsFeatureAllowed(SpreadsheetFeature.Clipboard))
        ...
        if (canCutPaste)
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_Paste)), Value = "paste", Icon = "content_paste" });
        }
        if (IsFeatureAllowed(SpreadsheetFeature.Editing))
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_ClearContents)), Value = "clear", Icon = "clear" });
        }
        if (IsFeatureAllowed(SpreadsheetFeature.Sorting))
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_ContextSortAscending)), Value = "sort-ascending", Icon = "arrow_upward" });
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_ContextSortDescending)), Value = "sort-descending", Icon = "arrow_downward" });
        }
    ...
    public async Task OnRowContextMenuAsync(CellEventArgs args)
    {
        ...
        if (IsFeatureAllowed(SpreadsheetFeature.Editing))
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_InsertRowAbove)), Value = "insert-row-before", Icon = "north" });
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_InsertRowBelow)), Value = "insert-row-after", Icon = "south" });
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_DeleteRow)), Value = "delete-row", Icon = "delete" });
        }
        if (IsFeatureAllowed(SpreadsheetFeature.Resizing))
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_HideRow)), Value = "hide-row", Icon = "visibility_off" });
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_UnhideRow)), Value = "unhide-row", Icon = "visibility" });
        }
    ...
    public async Task OnColumnContextMenuAsync(CellEventArgs args)
    {
        ...
        if (IsFeatureAllowed(SpreadsheetFeature.Editing))
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_InsertColumnBefore)), Value = "insert-column-before", Icon = "west" });
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_InsertColumnAfter)), Value = "insert-column-after", Icon = "east" });
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_DeleteColumn)), Value = "delete-column", Icon = "delete" });
        }
        if (IsFeatureAllowed(SpreadsheetFeature.Resizing))
        {
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_HideColumn)), Value = "hide-column", Icon = "visibility_off" });
            menuItems.Add(new() { Text = Localize(nameof(CrestStrings.Spreadsheet_UnhideColumn)), Value = "unhide-column", Icon = "visibility" });
        }
    ...
    private DotNetObjectReference<CrestSpreadsheet>? dotNetRef;
