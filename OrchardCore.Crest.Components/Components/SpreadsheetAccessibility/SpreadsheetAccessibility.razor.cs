...
/// <summary>
/// Isolated, perf-safe screen-reader announcer for <see cref="CrestSpreadsheet"/>. It binds to the
...
public partial class SpreadsheetAccessibility : ComponentBase, IDisposable
{
    ...
    private void OnSelectionChanged()
    {
        ...
        if (!range.Collapsed)
        {
            rangeText = string.Format(CultureInfo.CurrentCulture,
                Localize(nameof(CrestStrings.Spreadsheet_A11ySelection)),
    ...
    /// <param name="localize">Resolves a <see cref="CrestStrings"/> key to localized text.</param>
    public static string BuildAnnouncement(Worksheet worksheet, CellRef cell, Func<string, string> localize)
    {
        ...
        parts.Add(string.IsNullOrEmpty(content)
            ? localize(nameof(CrestStrings.Spreadsheet_A11yBlank))
            : content);
        ...
        {
            parts.Add(localize(nameof(CrestStrings.Spreadsheet_A11yMerged)));
        }
        ...
        if (c is not null)
        {
            ...
            {
                parts.Add(localize(nameof(CrestStrings.Spreadsheet_A11yFormula)));
            }
            if (c.ValueType == CellDataType.Error)
            {
                parts.Add(localize(nameof(CrestStrings.Spreadsheet_A11yError)));
            }
            if (c.Hyperlink is not null)
            {
                parts.Add(localize(nameof(CrestStrings.Spreadsheet_A11yHyperlink)));
            }
            if (worksheet.Protection.IsProtected && c.Format.IsLocked)
            {
                parts.Add(localize(nameof(CrestStrings.Spreadsheet_A11yReadOnly)));
            }
        }
        if (worksheet.FilteredColumns.Contains(cell.Column))
        {
            parts.Add(localize(nameof(CrestStrings.Spreadsheet_A11yFiltered)));
        }
