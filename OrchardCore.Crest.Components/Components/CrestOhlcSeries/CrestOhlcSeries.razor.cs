...
{
    /// <summary>
    /// A chart series that displays financial data as OHLC (Open-High-Low-Close) bars in a CrestChart.
    /// Each bar shows a vertical line from High to Low, with a left tick at the Open price and a right tick at the Close price.
    /// Similar to <see cref="CrestCandlestickSeries{TItem}"/> but uses tick marks instead of filled bodies.
    /// </summary>
    /// <typeparam name="TItem">The type of data items in the series. Each item represents one OHLC bar.</typeparam>
    /// <example>
    /// <code>
    /// &lt;CrestChart&gt;
    ///     &lt;CrestOhlcSeries Data=@stockData CategoryProperty="Date"
    ...
    public partial class CrestOhlcSeries<TItem> : CartesianSeries<TItem>
    {
        /// <summary>
