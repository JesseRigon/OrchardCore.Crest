...
/// </summary>
public partial class CrestToc : CrestComponent, IAsyncDisposable
{
    /// <summary>
    ...
    private readonly List<CrestTocItem> items = [];
    private IJSObjectReference? scrollListenerRef;

    internal async Task AddItemAsync(CrestTocItem item)
    {
        items.Add(item);
    ...
    internal async Task RemoveItemAsync(CrestTocItem item)
    {
        items.Remove(item);
